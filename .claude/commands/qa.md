...
- Is every navigation `router.push()`/`router.replace()` wired to a real route?
...
After completing the audit, report findings in this exact structure:
...
After the report, update `_project-docs/progress/qa-bugs.md`:
- Mark any previously listed issues that are now fixed with ~~strikethrough~~ and "✅ Fixed YYYY-MM-DD"
- Add any new issues found as new rows
- Update the tracking counts at the bottom of the file
